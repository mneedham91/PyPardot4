...
class Prospects(object):
	...
	def delete_by_email(self, email=None, **kwargs):
		results = self.read_by_email(email=email)
		rv = []
		if not results:
			raise PardotAPIArgumentError('email [{}] not found.'.format(email))
		if isinstance(results['prospect'], dict):
			rv.append(self.delete_by_id(results['prospect']['id']))
		else:
			for result in results['prospect']:
				rv.append(self.delete_by_id(result['id']))
		return rv
