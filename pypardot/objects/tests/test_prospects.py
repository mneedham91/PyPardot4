...
class TestProspects(unittest.TestCase):
	...
	def test_create_and_read(self):
		with self.assertRaises(PardotAPIArgumentError):
			results = self.pardot.prospects.create(first_name=self.first_name, last_name=self.last_name)

		results = self.pardot.prospects.create(
			email=self.email_address, first_name=self.first_name,
	...
	def test_update(self):
		results = self.pardot.prospects.create(email=self.email_address, first_name=self.first_name, last_name='McGee')
		prospect = results['prospect']
		self.assertEquals('McGee', prospect['last_name'])

		with self.assertRaises(PardotAPIArgumentError):
			self.pardot.prospects.update_by_id(id=None, last_name='Ferdle')

		self.pardot.prospects.update_by_id(id=prospect['id'], last_name='Ferdle')
	...
	def test_delete(self):
		...
		self.assertTrue(self.pardot.prospects.delete_by_id(id=prospect['id']))
